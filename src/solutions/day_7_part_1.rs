...
fn solution(calibration_equations: Vec<(i128,Vec<i128>)>)->i128{
    ...
    let operations = calibration_equation_len.iter().map(|x| {
        let base_operations = vec![Operation::ADD, Operation::MULTIPLY];
    ...
    let total_calibration_result = calibration_equations.iter().zip(operations).map(|(ce, o)| {
        let result = o.iter().map(|op_list|{
                ...
                // If we've found any equation that satisfies, sum the equation numbers.
                // else returns zero as this sum
                if found_result{
                    operations_result
                } else {
                    0
        ...
        let final_result: i128 = total_calibration_result.iter().map(|x| {
            match x.iter().filter(|x| **x!=0).next() {
                Some(n) => n,
                None => &0
            }
        }).sum();
